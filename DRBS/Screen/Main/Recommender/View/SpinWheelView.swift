...
// 원의 테두리를 설정하는 코드
        _ = CALayer().then {
            $0.frame = CGRect(x: 0, y: 0, width: self.bounds.width - ringLineWidth, height: self.bounds.height - ringLineWidth)
            $0.contentsScale = self.contentsScale
            $0.contents = ringImage.cgImage
            $0.contentsGravity = .center
            self.addSublayer($0)
...
class SliceLayer: CALayer {
    ...
    private func addTextLayer() {
        ...
        _ = CATextLayer().then {
            $0.frame = CGRect(x: center.x, y: 0, width: frame.width / 2, height: 15)
            $0.anchorPoint = CGPoint(x: 0, y: 0.5)
            $0.position = CGPoint(x: center.x, y: center.y)
            ...
            $0.transform = CATransform3DMakeAffineTransform(CGAffineTransform(rotationAngle: (endAngle - degreeOfSlice / 2).toRadian()))
        }
    }
}
